	...
	Convey("Given an environment with no environment variables set", t, func() {
		...
		Convey("When the config values are retrieved", func() {
			...
			Convey("The values should be set to the expected defaults", func(c C) {
				...
				So(cfg.KafkaConfig.Version, ShouldResemble, "1.0.2")
				So(cfg.KafkaConfig.NumWorkers, ShouldEqual, 1)
				So(cfg.KafkaConfig.FilterConsumerTopic, ShouldEqual, "filter-job-submitted")
				So(cfg.KafkaConfig.FilterConsumerGroup, ShouldEqual, "dp-dataset-exporter")
				So(cfg.FilterAPIURL, ShouldEqual, "http://localhost:22100")
				So(cfg.KafkaConfig.CSVExportedProducerTopic, ShouldEqual, "common-output-created")
